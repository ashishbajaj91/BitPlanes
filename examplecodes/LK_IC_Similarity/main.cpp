/**
 * Lucas Kanade Inverse Compositional using similarity warp
 */
...
// Source image is the template image to be found in target
void Align(cv::Mat &source, cv::Mat &target, int max_iterations, Eigen::Matrix3f &transformation)
{
    ...
    // Create images.
    source_gradient_row = cv::Mat(source.rows, source.cols, CV_32FC1); // CV_32FC1 defines depth and number of channels
    source_gradient_col = cv::Mat(source.rows, source.cols, CV_32FC1);
    steepest_descent = cv::Mat(source.rows, source.cols, CV_32FC4);  // 4 channels

    // The magic number 1/32 is inverse of the absolute sum of the weights in matrix
    // representing the Scharr filter (calculate gradient of the image)
    ...
    for(int row = 0; row < source.rows; row++)//
    {
        ...
        {
            // Evaluate image gradient 1x2 matrix
            ...
            // TODO: Check equation for similarity warp
            // similarity warp jacobian. 2x4 matrix
    ...
    // Hessian is only calculated once
    float alpha = 1e-4;
    H <<    h00+alpha ,h01      ,h02      ,h03,
    ...
    while(iter < max_iterations)
    {
        ...
        for(int row = 0; row < source.rows; row++)
        {
            ...
            for(int col = 0; col < source.cols; col++)
            {
                    ...
                    col2i >=0 && col2i < target.cols)
                {
                    ...
                    // Calcualte image difference D = I(W(x,p)) - T(x).
                    ...
                    // steepest descent * image difference
                    db *= (fabsf(D) < HUBER_LOSS ? D : D*HUBER_LOSS/fabsf(D));

                    // check if db can be used for later p = b \ H step.
        ...
        delta_p = H.ldlt().solve(b); // solving for p = b \ H
        ...
        // TODO: Check why Rodrigues' formula is used
        // Rodrigues' formulat
...
{
    cv::Mat img_src_c = cv::imread(argv[1], CV_LOAD_IMAGE_ANYDEPTH | CV_LOAD_IMAGE_ANYCOLOR);
    cv::Mat img_trg_c = cv::imread(argv[2], CV_LOAD_IMAGE_ANYDEPTH | CV_LOAD_IMAGE_ANYCOLOR);

    float x_offset = 0;
    float y_offset = 0;
    if(argc == 5)
    {
        x_offset = atof(argv[3]); // atof: convert string to double
